...
{ TKMBuilderKP }
constructor TKMBuilderKP.Create(aOnLog, aOnLogVerbose: TProc<string>; aOnStepBegin: TKMEventStepBegin; aOnStepDone, aOnStepFail: TKMEventStepDone; aOnDone: TProc);
begin
  inherited;
...
begin
  ...
  DeleteRecursive(ExpandFileName('.\'), [
    ...
    'thumbs.db', 'descript.ion', 'bugreport.txt', '*.skincfg', '*.identcache', '*.tvsconfig', '*.mi', '*.log.txt', '*.stat', '*.bak'], ['.git', 'Builder.log']);
end;
